@php
    ...
    $grid = 'display:grid;grid-template-columns:minmax(150px,1.5fr) 0.9fr 1.1fr 0.9fr 1.15fr 1.7fr 1.3fr;align-items:center;gap:12px;';
...
<div class="-m-6 min-h-screen bg-[#f4f5f7] p-4 text-[13px] text-[#3d424b] sm:px-10 sm:py-8"
    ...
    <div class="mx-auto max-w-[1200px]">
        ...
        {{-- Filter / sort panel --}}
        <div class="mb-[18px] flex flex-col gap-2.5 rounded-2xl border border-[#e8eaef] bg-white p-3.5">
            {{-- filters --}}
            <div class="flex items-center gap-3">
                <div class="w-[120px] flex-none rounded-[9px] bg-[#eef0f4] py-[11px] text-center text-[13px] font-bold text-[#5b616c]">{{ __('Filter') }}</div>
            ...
            {{-- sort --}}
            <div class="flex items-center gap-3">
                <div class="w-[120px] flex-none rounded-[9px] bg-[#eef0f4] py-[11px] text-center text-[13px] font-bold text-[#5b616c]">{{ __('Sort') }}</div>
                <div class="flex flex-1 flex-wrap gap-3">
                    ...
                    <label class="flex min-w-[160px] flex-1 cursor-pointer items-center gap-2.5 rounded-[9px] border border-[#e6e8ed] px-3.5 py-2.5 text-[13px] font-semibold text-[#3d424b]">
        ...
        <div class="overflow-hidden rounded-2xl border border-[#e8eaef] bg-white">
            ...
            @if (count($rows) > 0)
                ...
                <div class="border-b border-[#f1f2f5] bg-[#fbfcfd] px-[22px] py-3.5" style="{{ $grid }}">
                    <span class="text-[14px] font-bold tabular-nums text-[#171a1f]">{{ number_format($summary['count']) }}</span>
                    <span class="text-[13.5px] font-bold tabular-nums text-[#171a1f]">{{ number_format($summary['shares']) }}</span>
                    <span class="text-[13.5px] font-bold tabular-nums text-[#171a1f]">{{ number_format($summary['total']) }}</span>
                ...
                @foreach ($rows as $row)
                    <div wire:key="report-row-{{ $row['id'] }}">
                        ...
                        <div wire:click="toggle({{ $row['id'] }})"
                            class="cursor-pointer border-b border-[#f1f2f5] px-[22px] py-[15px] transition hover:bg-[#fafbfc]" style="{{ $grid }}">
                            <span class="flex flex-wrap items-baseline gap-1.5">
                                <span class="text-[14px] font-bold text-[#171a1f]">{{ $row['name'] }}</span>
                                @if ($row['idNum'])
                                    <span class="text-[12.5px] font-medium tabular-nums text-[#8b919c]">{{ $row['idNum'] }}</span>
                                @endif
                                @if ($row['gender'])
                                    <span class="text-[12.5px] font-semibold text-primary">({{ $row['gender'] }})</span>
                                @endif
                            </span>
                            <span class="text-[13px] tabular-nums text-[#3d424b]">{{ $row['shares'] !== null ? number_format($row['shares']) : '—' }}</span>
                            <span class="text-[13px] tabular-nums text-[#3d424b]">{{ $row['total'] !== null ? number_format($row['total']) : '—' }}</span>
                            <span class="text-[13px] text-[#3d424b]">{{ $row['worker'] }}</span>
                            <span class="text-[12.5px] tabular-nums text-[#5b616c]">{{ $row['submitDate']?->format('y/m/d H:i') ?? '—' }}</span>
                            <span class="truncate text-[13px] text-[#5b616c]">{{ $project->title }}</span>
                            <span class="flex items-center justify-end gap-2.5">
                                <span class="text-[11.5px] tabular-nums text-[#aeb4be]">{{ $row['judgeDate']?->format('y/m/d') ?? '' }}</span>
                                <span class="inline-flex items-center whitespace-nowrap rounded-full px-3 py-1 text-[11.5px] font-semibold {{ $row['chip'] }}">{{ $row['judgment'] }}</span>
                            </span>
                        </div>

                        {{-- expandable detail --}}
                        @if ($row['isOpen'])
                            <div class="grid grid-cols-1 gap-7 border-b border-[#f1f2f5] bg-[#fafbfc] px-[26px] pb-6 pt-5 md:grid-cols-2">
                                <div class="flex flex-col gap-[11px]">
                                    <div class="flex gap-2.5"><span class="w-[84px] flex-none text-[12px] font-semibold text-[#8b919c]">{{ __('Created') }}</span><span class="tabular-nums text-[13px] text-[#3d424b]">{{ $row['detail']['created']?->format('Y-m-d H:i') ?? '—' }}</span></div>
                                    <div class="flex gap-2.5"><span class="w-[84px] flex-none text-[12px] font-semibold text-[#8b919c]">{{ __('Activity date') }}</span><span class="tabular-nums text-[13px] text-[#3d424b]">{{ $row['detail']['actDate']?->format('Y-m-d') ?? '—' }}</span></div>
                                    <div class="flex gap-2.5"><span class="w-[84px] flex-none text-[12px] font-semibold text-[#8b919c]">{{ __('Address') }}</span><span class="text-[13px] leading-relaxed text-[#3d424b]" style="text-wrap:pretty;">{{ $row['detail']['address'] }}</span></div>
                                    <div class="flex gap-2.5"><span class="w-[84px] flex-none text-[12px] font-semibold text-[#8b919c]">{{ __('Contact information') }}</span><span class="tabular-nums text-[13px] text-[#3d424b]">{{ $row['detail']['contact'] }}</span></div>
                                </div>
                                <div class="flex flex-col gap-3.5">
                                    <div>
                                        <div class="mb-2 text-[11px] font-semibold tracking-wide text-[#8b919c]">{{ __('Files') }}</div>
                                        @forelse ($row['detail']['files'] as $file)
                                            <div class="mb-2 flex items-center justify-between gap-3 rounded-[10px] border border-[#e6e8ed] bg-white px-3 py-2.5">
                                                <span class="flex min-w-0 items-center gap-2.5 text-[13px] font-medium text-[#3d424b]">
                                                    <x-heroicon-o-document class="size-[15px] flex-none text-[#9aa0aa]" />
                                                    <span class="truncate">{{ $file['name'] }}</span>
                                                </span>
                                                <span class="flex flex-none gap-1.5">
                                                    <a href="{{ route('reports.file', ['submission' => $row['id'], 'index' => $file['index'], 'preview' => 1]) }}" target="_blank" rel="noopener"
                                        ...
                                        @empty
                                            <div class="rounded-[10px] border border-dashed border-[#e6e8ed] px-3 py-2.5 text-[12.5px] text-[#aeb4be]">{{ __('No files') }}</div>
                                        @endforelse
                                    </div>
                                    <div>
                                        <div class="mb-2 text-[11px] font-semibold tracking-wide text-[#8b919c]">{{ __('Notes') }}</div>
                                        <div class="text-[13.5px] leading-relaxed text-[#3d424b]" style="text-wrap:pretty;">{{ $row['detail']['note'] }}</div>
                                    </div>
                                </div>
                            </div>
                        @endif
                ...
                @endforeach
            @else
                <div class="px-5 py-16 text-center text-sm font-medium text-[#aeb4be]">{{ __('No reports for this project yet.') }}</div>
            @endif
        </div>
