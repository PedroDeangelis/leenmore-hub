...
<div class="-m-6 min-h-screen bg-[#f4f5f7] p-4 text-[13px] text-[#3d424b] sm:px-10 sm:py-8"
    ...
    <div class="mx-auto max-w-[1200px]">
        <h1 class="mb-2 text-[26px] font-bold tracking-tight text-[#171a1f]">{{ __('Reports') }}</h1>
        <p class="mb-6 text-sm text-[#8b919c]">{{ __('Projects with activity reports') }}</p>
                ...
                <a href="{{ route('reports.show', $project) }}" wire:navigate wire:key="report-project-{{ $project->id }}"
                    ...
                    <div class="flex min-w-0 flex-wrap items-baseline gap-3">
                        ...
                        @if ($project->last_report_at)
                            <span class="text-[12.5px] font-medium tabular-nums text-[#aeb4be]">{{ Carbon::parse($project->last_report_at)->format('Y/m/d H:i') }}h</span>
                        @endif
                    </div>
                    <div class="flex flex-none items-center gap-5">
                        ...
                        @endphp
                        <span class="whitespace-nowrap text-[13px] font-semibold" style="color:{{ $statusColor }};">{{ $project->status->label() }}</span>
                    </div>
                </a>
            @empty
