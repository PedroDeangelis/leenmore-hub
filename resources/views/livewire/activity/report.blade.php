<div class="">
    ...
    <div class="mb-6">
        <h1 class="text-xl font-semibold text-zinc-900">{{ __('Activity report') }}</h1>
    ...
    <div class="">
        ...
        <div class="grid items-start gap-4 lg:grid-cols-2">
            ...
            <div class="space-y-4">
                    ...
                    <div class="flex items-start justify-between gap-4 border-b border-[#f1f2f5] pb-4">
                        <div>
                            ...
                            <div class="flex flex-wrap items-baseline gap-2">
                                <span
                                    class="text-[18px] font-bold text-[#171a1f]">{{ $shareholder?->name ?? '—' }}</span>
                                <span
                                    class="text-[13px] font-medium tabular-nums text-[#8b919c]">{{ $shareholder?->date_of_birth_code ?: $shareholder?->registration }}</span>
                            </div>
                        </div>
                        @if ($shareholder?->sex)
                    ...
                    <div class="flex items-end justify-between gap-4 pt-4">
                        ...
                        <div class="flex flex-none gap-6 text-right">
                            <div>
                                ...
                                <div class="text-[15px] font-bold tabular-nums text-[#171a1f]">
                                    {{ $projectShareholder->shares !== null ? number_format($projectShareholder->shares) : '—' }}
                                </div>
                            </div>
                            <div>
                                <div class="mb-1 text-[11.5px] font-semibold text-[#8b919c]">{{ __('Total shares') }}
                                </div>
                                <div class="text-[15px] font-bold tabular-nums text-[#171a1f]">
                ...
                {{-- Previous reports --}}
                <div class="rounded-2xl border border-[#e8eaef] bg-white p-5">
                    <div class="mb-3 text-[11.5px] font-semibold uppercase tracking-wide text-[#8b919c]">
                        {{ __('Previous reports') }}</div>
                    <div class="space-y-2.5">
                        @forelse ($previousSubmissions as $sub)
                            ...
                            <div wire:key="prev-{{ $sub->id }}" class="rounded-xl border border-[#eef0f3] p-3.5">
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        ...
                                        @if ($sub->created_by_user_id)
                                            <span
                                                class="inline-flex items-center gap-1 rounded-full bg-[#eef0f4] px-2 py-0.5 text-[10.5px] font-semibold text-[#7d838f]"
                                                title="{{ $sub->creator?->name }}">
                                                <x-heroicon-o-pencil
                                                    class="size-3" />{{ __('Manually entered (admin)') }}
                                            </span>
                                        @endif
                                    </div>
                                    <div class="flex shrink-0 items-center gap-2">
                                        <span
                                            class="text-[12px] tabular-nums text-[#aeb4be]">{{ ($sub->date ?? $sub->created_at)->format('Y/m/d') }}</span>
                                ...
                                @if ($sub->contact)
                                    <div class="mt-2 text-[12.5px] font-semibold text-primary">
                                        {{ __('Contact information') }}: {{ $sub->contact }}</div>
                                @endif
                                @if ($sub->note)
                                    <p class="mt-1.5 text-[13px] leading-relaxed text-[#3d424b]">{{ $sub->note }}
                                    </p>
                                @endif
                            </div>
                        @empty
            ...
            <div class="space-y-4">
                @unless ($showForm)
                    <div
                        class="flex items-center gap-2.5 rounded-2xl border border-[#bfe6cf] bg-[#e9f7ef] px-5 py-4 text-[14px] font-semibold text-[#15834a]">
                ...
                <div class="rounded-2xl border border-[#e8eaef] bg-white p-5">
                    <div class="relative">
                        ...
                        <x-heroicon-o-chevron-down
                            class="pointer-events-none absolute end-3.5 top-1/2 size-4 -translate-y-1/2 text-[#9aa0aa]" />
                    </div>
                    ...
                    @if ($showForm)
                        <div class="mt-5 space-y-4 border-t border-[#f1f2f5] pt-5">
                            @if ($editingId)
                                <div
                                    class="rounded-lg bg-[#fff7e8] px-3.5 py-2.5 text-[12.5px] font-semibold text-[#9a7406]">
                                    {{ __('Editing an existing report') }}</div>
                            @endif

                            {{-- Existing attachments (edit mode) --}}
                            @if (count($existingAttachments))
                                <div class="space-y-1.5">
                                    <div class="text-[11px] font-semibold uppercase tracking-wide text-[#8b919c]">
                                        {{ __('Attached files') }}</div>
                                    @foreach ($existingAttachments as $i => $path)
                                        <div wire:key="existing-{{ $i }}"
                                            class="flex items-center justify-between gap-2 rounded-lg border border-[#e6e8ed] px-3 py-2 text-[12.5px]">
                                            ...
                                            <button type="button"
                                                wire:click="removeExistingAttachment({{ $i }})"
